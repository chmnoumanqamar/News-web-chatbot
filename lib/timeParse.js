...
export function extractTimeSlot(message) {
  ...
  // Match 9 baje / 9 bajhay / 9 bajy / 9 bje / 10 baje
  const bajeMatch = msg.match(/\b(\d{1,2})\s*(?:baje|bajhay|bajy|bje|bjay)\b/i);
  if (bajeMatch) {
    const hour = Number(bajeMatch[1]);
    const isNight = /raat|shaam|evening|night/.test(msg);
    const ampm = isNight || (hour >= 6 && hour <= 11) || hour === 12 ? "PM" : "AM";
    return {
      slot: `${hour} ${ampm}`,
      slotAlt: `${hour}${ampm}`,
      hour,
  ...
  for (const [w, h] of Object.entries(wordMap)) {
    if (new RegExp(`\\b${w}\\s*(?:baje|bajhay|bajy|bje)\\b`, "i").test(msg)) {
      const isNight = /raat|shaam|evening|night/.test(msg);
      const ampm = isNight || (h >= 6 && h <= 11) || h === 12 ? "PM" : "AM";
      return {
        slot: `${h} ${ampm}`,
        slotAlt: `${h}${ampm}`,
        hour: h,
